...
def parseMovements(movement):
    return [
        int(i) for i in movement
            .replace("move ", "")
            .replace("from ", "")
            .replace("to ", "")
            .split(" ")
...
'''
def solve01(data):
    lines, movements = [info.split('\n') for info in data.split('\n\n')]
    
    stacks = buildStacks(lines)
    
    for movement in movements:
        move, of, to = parseMovements(movement)
        
        for _ in range(0, move):
            lastFrom = stacks[of].pop()
            stacks[to].append(lastFrom)
...
def solve02(data):
    ...
    for movement in movements:
        ...
        for _ in range(0, move):
            lastFrom = stacks[of].pop()
            tempStack.append(lastFrom)
        
        for i in range(move - 1, -1, -1):
            stacks[to].append(tempStack[i])
