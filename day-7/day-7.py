...
class Directory:
    ...
    def getSubDirectoryByName(self, name):
    ...
    def getFullSize(self):
        total = self.getFileSize()
        
        if not self.hasDirectories():
            return total

        for directory in self.getDirectories():
            total += directory.getFullSize()
        return total
...
def buildDirectoryTree(lines):
    ...
    for line in lines:
        if line == "$ cd /":
            ...
            name = line.split(" ")[1]
            newDirectory = Directory(name, currentDirectory)
            currentDirectory.addDirectory(newDirectory)
            allDirectories.append(newDirectory)
        elif "$ cd" in line:
            name = line.split(" ")[2]
            currentDirectory = currentDirectory.getSubDirectoryByName(name)
        else:
            # is file!
            size, name = line.split(" ")
            file = SystemFile(name, int(size))
...
def solve01(data):
    lines = data.split("\n")
    _, allDirectories = buildDirectoryTree(lines)
...
Your puzzle answer was 6400111.
'''
def solve02(data):
    MAX_SIZE = 70000000
    ...
    lines = data.split("\n")
    root, allDirectories = buildDirectoryTree(lines)

    unusedSize = MAX_SIZE - root.getFullSize()
    atLeastSize = NEEDED_SIZE - unusedSize
